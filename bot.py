...
# Channel IDs (numeric, e.g. -100xxxxxxxxxx) or @username — bot must be admin
...
def sanitize_filename(name: str) -> str:
    """Safe filesystem / Telegram document name."""
    name = re.sub(r'[\\/:*?"<>|]+', " ", name)
...
def build_episode_caption(
    ...
    return (
        f"<b>{html_escape(title)} - {html_escape(ep_num)}</b>\n"
...
def parse_chat_id(raw: Optional[str]):
    """Parse channel id or @username from env."""
    if not raw:
        return None
    s = raw.strip()
    if not s:
...
async def send_photo_no_reply(
    client: Client,
    chat_id,
    photo: str,
    caption: str = "",
) -> None:
    """Post poster without replying to the user's link message."""
...
async def send_document_no_reply(
    ...
    """Upload file without reply_to (no quote of the original link)."""
    kwargs = dict(
        chat_id=chat_id,
        document=document,
        file_name=file_name,
...
def media_destinations(fallback_chat_id: int) -> list:
    """
    Primary = UPLOAD_CHANNEL (or user chat if unset).
    DUMP_CHANNEL is an extra copy if set and different.
    """
    dests = []
...
async def process_urls(client: Client, message: Message, urls: list[str]) -> None:
    ...
    for s_idx, series_key in enumerate(order, 1):
        ...
        # Where media goes: UPLOAD_CHANNEL (preferred) / DUMP_CHANNEL / user chat
        dest_chats = media_destinations(message.chat.id)

        # Local JPEG thumb for documents (Aeon-style): series poster preferred
        ...
        # Poster + series card first in upload channel(s) — no reply to link
        for chat_id in dest_chats:
            try:
                if series_info.poster_url:
                    await send_photo_no_reply(
                        client, chat_id, series_info.poster_url, series_caption
                    )
                else:
        ...
        for url in series_urls:
            ...
            # Document thumbnail (Aeon leech style): poster → else video frame
            ...
            for chat_id in dest_chats:
                try:
                    ...
                    logger.exception("Upload failed to %s for %s", chat_id, final_path)
                    await message.reply(
                        f"⚠️ Upload failed to <code>{chat_id}</code>: {e}\n"
                        f"<code>{final_path.name}</code>",
                        parse_mode=enums.ParseMode.HTML,
                    )
