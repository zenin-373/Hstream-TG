...
async def process_urls(client: Client, message: Message, urls: list[str]) -> None:
    ...
    for s_idx, series_key in enumerate(order, 1):
        ...
        # Never use site SEO junk as the series title
        def _bad_title(t: str) -> bool:
            if not t:
                return True
            tl = t.lower()
            return any(x in tl for x in (
                "watch english subbed", "hstream.moe", "hentai online in hd",
                "stream english subbed",
            ))

        raw_slug = series_key.rstrip("/").split("/")[-1]
        slug_title = " ".join(w.capitalize() for w in raw_slug.replace("_", "-").split("-") if w) or raw_slug

        if _bad_title(series_info.title):
            series_info.title = slug_title

        series_caption = build_series_caption(series_info, has_subs=True)
        title_label = series_info.title or slug_title
        anime_title = series_info.title or slug_title
