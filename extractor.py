...
def _is_generic_site_title(title: str) -> bool:
    """True if title is the site-wide SEO/default, not a real series name."""
    if not title:
        return True
    t = title.lower().strip()
    bad = (
        ...
        "hstream.moe",
        "stream english subbed",
        "hentai online in hd",
        "browse series",
    )
    return any(b in t for b in bad) or t in {"hstream", "hstream.moe"}


def slug_to_title(slug: str) -> str:
    """nikuyome / modaete-yo-adam-kun → readable title."""
    slug = re.sub(r"-\d+$", "", (slug or "").strip().strip("/"))
    slug = slug.split("/")[-1]
    if not slug:
        return "Unknown"
    parts = slug.replace("_", "-").split("-")
    return " ".join(w.capitalize() for w in parts if w).strip() or "Unknown"
...
def scrape_series_info(
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            token = series_url.rstrip("/").split("/")[-1]
            info.title = slug_to_title(token)
            return info
        page = r.text
    except Exception as e:
    ...
    if m:
        candidates.append(re.sub(r"<[^>]+>", "", m.group(1)).strip())
    for pat in (
    ...
    for c in candidates:
        c = re.sub(r"\s*[|–—-]\s*hstream\.moe.*$", "", c, flags=re.I).strip()
        c = re.sub(r"\s*-\s*Watch All.*$", "", c, flags=re.I).strip()
        if c and not _is_generic_site_title(c):
            info.title = c
            break

    if not info.title or _is_generic_site_title(info.title):
        token = series_url.rstrip("/").split("/")[-1]
        info.title = slug_to_title(token)
    ...
    if not info.poster_url:
        ...
        if m:
            img = m.group(1).strip()
            if img and "cropped-HS" not in img and "default-avatar" not in img and "/images/hentai/" in img:
                info.poster_url = img
